...
- `config/brand.ts` is live-real: contact gunlicensenyc@gmail.com / (929) 352-5961; `domain`/`url` = gunlicensenyc.com. The site is on the custom domain **gunlicensenyc.com** (Vercel project `ccw`, apex A→76.76.21.21 + www CNAME→cname.vercel-dns.com on Porkbun; `NEXT_PUBLIC_SITE_URL=https://gunlicensenyc.com`). `ccw-eight.vercel.app` still resolves too.
