...
export async function getMyCase() {
  ...
  const { data } = await supabase
    .from("cases")
    .select(
      "id, stage, status, service_mode, calendly_token, nypd_app_ref, target_file_date, license_expires_on, is_renewal, license_track, client_id, clients(id, full_name, email, phone, borough, zip, track)"
    )
