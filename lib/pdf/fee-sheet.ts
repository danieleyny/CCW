...
export async function renderFeeSheet(args: {
  ...
  return buildPdf(
    (c) => {
      ...
      c.keepTogether(120)
      c.h2("Booking your fingerprinting")
      c.para(`${FINGERPRINT_SCHEDULING.vendor} · ${FINGERPRINT_SCHEDULING.phone}`, LINE)
      c.para(`Book: ${FINGERPRINT_SCHEDULING.schedulingUrl}`, LINE)
      c.para(`Service codes: ${FINGERPRINT_SCHEDULING.lookupUrl}`, LINE)
      c.para(FINGERPRINT_SCHEDULING.serviceCodeNote, LINE)
      c.spacer(2)
